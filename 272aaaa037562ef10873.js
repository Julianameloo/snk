...
const percent = (x) => (x * 100).toFixed(5);
const createSnake = (chain, { sizeCell }, duration) => {
    ...
    const styles = [
      ...
      animation: none linear ${duration}ms infinite;
        ...
        ...snakeParts.map((positions, i) => {
            ...
            const id = `s${i}`;
            const animationName = id;
            let p2 = removeInterpolatedPositions(positions.map((tr, i, { length }) => ({ ...tr, t: i / length, i }))).map((p) => ({
                x: p.x,
                y: p.y,
                i: p.i,
                t: p.t,
            ...
            return [
                `@keyframes ${animationName} { ` +
                    removeInterpolatedPositions(positions.map((tr, i, { length }) => ({ ...tr, t: i / length, i })))
                        .map((p, i) => {
                        var pi = i + 1;
                        if (pi === p2.length) {
                            pi = 0;
...
const onSubmit = async (userName) => {
    ...
    const chain = (0,_snk_solver_getBestRoute__WEBPACK_IMPORTED_MODULE_0__.getBestRoute)(grid, _snk_types_fixtures_snake__WEBPACK_IMPORTED_MODULE_6__.snake1);
    chain.push(...(0,_snk_solver_getPathToPose__WEBPACK_IMPORTED_MODULE_7__.getPathToPose)(chain.slice(-1)[0], _snk_types_fixtures_snake__WEBPACK_IMPORTED_MODULE_6__.snake1));
    dispose();
